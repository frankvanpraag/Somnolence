...
struct ContentView: View {
    ...
    enum NotificationIssue: Identifiable {
        ...
        var message: String {
            ...
            case .volumeLow:
                return "System volume is too low to preview sounds"
            case .dndEnabled:
...
//                }
    ...
    var body: some View {
        ...
        .sheet(isPresented: $showingNotificationWarning) {
            NavigationView {
                List {
                    Section {
                        Text("Some settings may prevent alarms from working properly. Please review and adjust the following:")
                            .font(.subheadline)
                            .foregroundColor(.secondary)
                            .padding(.vertical, 8)
                    }
                    
                    ForEach(notificationIssues) { issue in
                ...
                .navigationTitle("Settings Review")
                .navigationBarTitleDisplayMode(.inline)
                .toolbar {
                    ToolbarItem(placement: .navigationBarTrailing) {
                        Button("Done") {
                            showingNotificationWarning = false
                        }
                    }
                }
            }
